...
def run_pipeline(audio_path: Path, work_dir: Path, out_path: Path, tempo: float = 120.0) -> Path:
    ...
    # Make sure our scratch/cache folder exists before anything tries
    # to write into it.
    ...
    # files exist), rather than going through the generic `_cached`
    # JSON helper used below.
    stem_set = stems.separate_stems(audio_path, work_dir)

    # --- Stage 2: figure out the song's beats and sections ---
    # `_cached(...)` (defined further down) either loads a
    ...
    # no drums/vocals) -- see `_write_harmonic_mix` below, which builds
...
# "__type__" back to the actual class objects, so `_from_jsonable`
# below knows which constructor to call.
_TYPES = {"Beat": Beat, "Section": Section, "NoteEvent": NoteEvent, "ChordEvent": ChordEvent}


def _from_jsonable(obj):
    """The reverse of `_to_jsonable`: turn plain JSON-loaded data back
